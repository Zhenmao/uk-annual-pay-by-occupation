...
d3.json("data/data.json", (error, json) => {
  ...
  const simulation = d3
    ...
    .force("y", d3.forceY((d) => y(d.median_4)).strength(1))
    .force(
      "collide",
      d3
        .forceCollide(circleRadius + circlePadding)
        .strength(1)
        .iterations(5)
    )
    .stop();
  ...
  medianLine.append("line").attr("x2", width);
  medianLine.append("text").attr("dy", "-0.32em").text("Median Annual Pay");
  medianLine
    .append("text")
    .attr("dy", "-0.32em")
  ...
  d3.select("#search-input").on("input", function () {
    const re = RegExp(this.value.trim(), "gi");
    circle.attr("opacity", (d) => (re.test(d.data.description_4) ? 1 : 0.2));
  });
});
